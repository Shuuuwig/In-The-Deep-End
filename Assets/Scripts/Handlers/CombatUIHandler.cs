...
{
    [SerializeField] protected BattleData _battleValuesSO;
    ...
    protected List<GameObject> _playerSpawnPos { get { return _turnController.PlayerSpawnPos; } }
    protected List<GameObject> _enemySpawnPos { get { return _turnController.EnemySpawnPos; } }
    ...
    public void InitializeHealthDisplay()
    {
        ...
        {
            for (int j = 0; j < _turnController.ActiveUnits.Count; j++)
            {
                if (_turnController.ActiveUnits[j].transform.parent.gameObject == _playerSpawnPos[i].gameObject)
                {
                    _playerHealthbars[i].maxValue = _turnController.ActiveUnits[j].MaxHealthPoints;
                    _playerHealthbars[i].minValue = 0;
                    _playerHealthbars[i].value = _turnController.ActiveUnits[j].CurrentHealthPoints;
                    Debug.Log("Initialized Player Health Display");
                }

                if (_turnController.ActiveUnits[j].transform.parent.gameObject == _enemySpawnPos[i].gameObject)
                {
                    _enemyHealthbars[i].maxValue = _turnController.ActiveUnits[j].MaxHealthPoints;
                    _enemyHealthbars[i].minValue = 0;
                    _enemyHealthbars[i].value = _turnController.ActiveUnits[j].CurrentHealthPoints;
    ...
    public void SaveSelectedTargets()
    {
        for (int i = 0; i < _markedTargetIndicators.Count; i++)
        {
            if (_currentTargetedPosition == _markedTargetIndicators[i].transform.parent.gameObject)
            {
                ...
                {
                    int spriteIndex = _currentActiveUnit.UnitData.MarkedTargetIndicatorSprites.IndexOf(_markedTargetIndicators[i].sprite);
                    _markedTargetIndicators[i].sprite = _currentActiveUnit.UnitData.MarkedTargetIndicatorSprites[spriteIndex + 1];
                }
                else
                {
                    _markedTargetIndicators[i].sprite = _currentActiveUnit.UnitData.MarkedTargetIndicatorSprites[0];
    ...
    public void UpdateTurnDisplay()
    {
        ...
        {
            if (i < _turnController.ActiveUnits.Count)
            {
                _turnValueText[i].text = $"{_turnController.ActiveUnits[i].name} TV: {_turnController.ActiveUnits[i].CurrentTurnValue}";
            }
            else
            {
